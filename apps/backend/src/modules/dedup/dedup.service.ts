...
function toSummary(article: ArticleSchema): string {
...
function similarityScore(article: ArticleSchema, candidate: NewsEvent): number {
...
export class DedupService {
  ...
  deduplicate(articles: ArticleSchema[]): NewsEvent[] {
    ...
    for (const article of articles) {
      ...
      existing.avgReliability = Math.round(
        (existing.avgReliability * previousCount + article.reliabilityScore) / (previousCount + 1)
      )
